...
go.merge = function(base, data) {
	var idx = base.idx || 0;
	for (var i in data) {
		if (i == "block" && (data[i] instanceof Array)) {
			if (typeof base[i] != 'object') base[i] = [];
			for (var n = 0; n < data[i].length; n++) {
				this.merge(base[idx + n], data[i]);
			}
		}else
		if (typeof data[i] == 'object') {
			if (typeof base[i] != 'object') base[i] = (data[i] instanceof Array)? [] : {};
			this.merge(base[i], data[i]);
		} else base[i] = data[i];
	}
	return base;
}
