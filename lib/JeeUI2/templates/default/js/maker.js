...
var render = function(){
	...
	fn_secrion = {
		...
		on_change: function(d, id) {
			...
			var dt = [{key:id}]
			if (value !== undefined) dt[0].val = value;
			ws.send_post(dt);
		},
		...
		on_submit: function(d, id) {
			var dt = [], data = go.formdata(go("input, textarea, select", go("#"+id)));
			for (var i in data) dt.push({key: i, val: data[i]});
			dt.push({key: id});
			ws.send_post(dt);
		}
	},
