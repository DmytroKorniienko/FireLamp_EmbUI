...
function content_item(item, i){
    ...
    if (item.type == "checkbox"){
        ...
        if(item.type == "range" || item.type == "text"){
            ...
            content += " oninput=\"datarange(this.id, this.value, '" + item.label + "')\">";
        }

        if(item.type == "password" || item.type == "number" || item.type == "time" || item.type == "date" || item.type == "datetime-local"){
    ...
    if(item.html == "textarea"){
        content += "<textarea class=\"pure-u-24-24\" maxlength=\"255\" id=\"" + item.id + "\" onchange=\"data('" + item.type + "', this.id, this.value, '" + item.label + "', " + page + ", " + i + ")\">" + item.value + "</textarea>";
    }
    if(item.html == "select"){
...
document.addEventListener("DOMContentLoaded", function () {
  //parse();    // избегаем двойного вызова /echo при каждом обновлении
...
//        if(needRefresh._refresh == "1"){
          if(needRefresh._tstamp != tstamp){
            tstamp = needRefresh._tstamp;
            parse();
