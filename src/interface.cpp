...
void interface()
{ // функция в которой мф формируем веб интерфейс
#ifdef MIC_EFFECTS
    ...
    if (1)
    {
        ...
        // создаем меню
        jee.menu(F("Управление"));
        if (!iGLOBAL.isSetup)
        {
            jee.menu(F("Время/Текст"));
        ...
        if (myLamp.getMode() == MODE_DEMO)
            jee.button(F("bDemo"), F("blue"), F("Выключить DEMO"));
        else
        ...
        if (!iGLOBAL.isSetup)
        {
            ...
            if (iGLOBAL.isAddSetup)
            {
                ...
                case 3:
                    ...
                    if (jee.param(F("isEdEvent")) == F("true"))
                    { // редактируем параметры событий
            ...
            else
            {
                ...
                String cfg(F("Конфигурации"));
                cfg += " (";
                cfg += jee.param(F("fileList"));
                cfg += ")";
...
void update()
{ // функция выполняется после ввода данных в веб интерфейсе. получение параметров из веб интерфейса в переменные
    ...
    { // для служебного "пустого" эффекта EFF_NONE вообще ничего не делаем
        //LOG(printf_P, PSTR("curEff: %p iGLOBAL.prevEffect: %p\n"), curEff, iGLOBAL.prevEffect);
        if (curEff != iGLOBAL.prevEffect && iGLOBAL.prevEffect != nullptr)
        { // Если эффект поменялся или требуется обновление UI, при этом не первый вход в процедуру после перезагрузки
            if (myLamp.isLampOn())
                myLamp.switcheffect(SW_SPECIFIC, myLamp.getFaderFlag(), curEff->eff_nb);
            else
            {
        ...
        { // эффект не менялся, либо MQTT, либо первый вход - обновляем текущий эффект значениями из UI/MQTT
            ...
            if (curEff->param == nullptr || strcmp_P(curEff->param, (jee.param(F("param"))).c_str()))
            { // различаются
            ...
            if (strcmp_P((jee.param(F("param"))).c_str(), curEff->param))
            { // различаются  || (curEff->param==nullptr && (jee.param(F("param"))).length()!=0)
                curEff->updateParam((jee.param(F("param"))).c_str());
            }
            ...
            if (iGLOBAL.prevEffect != nullptr)
            {
                if (!myLamp.effects.autoSaveConfig())
                {                                      // отложенная запись, не чаще чем однократно в 30 секунд
