...
void generateLine() {
  for (uint8_t x = 0U; x < WIDTH; x++) {
    GSHMEM.line[x] = random(127, 255);                             // заполнение случайным образом нижней линии (127, 255) - менее контрастное, (64, 255) - оригинал
  }
}
...
void drawFrame(uint8_t pcnt, bool isColored) {                  // прорисовка нового кадра
  ...
  deltaHue = random(0U, 2U) ? constrain (GSHMEM.shiftHue[0] + random(0U, 2U) - random(0U, 2U), 15U, 17U) : GSHMEM.shiftHue[0]; // random(0U, 2U)= скорость смещения языков чем больше 2U - тем медленнее
  ...
  deltaValue = random(0U, 3U) ? constrain (GSHMEM.shiftValue[0] + random(0U, 2U) - random(0U, 2U), 15U, 17U) : GSHMEM.shiftValue[0]; // random(0U, 3U)= скорость смещения очага чем больше 3U - тем медленнее
  // 15U, 17U - амплитуда качания -1...+1 относительно 16U
  ...
  for (uint8_t y = HEIGHT - 1U; y > 0U; y--) {                                      // прорисовка остальных строк с учетом значения низлежащих
    ...
    for (uint8_t x = 0U; x < WIDTH; x++) {                                          // пересчет координаты x для текущей строки
      ...
      if (y < 8U) {                                                                 // если строка представляет очаг
        ...
        if (random(0, 20) == 0 && myLamp.getPixColorXY(x, y - 1U) != 0U) myLamp.drawPixelXY(x, y, myLamp.getPixColorXY(x, y - 2U));  // 20 = обратная величина количества искр
