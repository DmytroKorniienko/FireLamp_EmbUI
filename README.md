...
## ESP8266 vs ESP32
На данный момент основная разработка ведется под контроллер esp8266, в мастер ветке сборка поддерживается только под эту платформу. Код для ESP32 находится в [отдельной ветке](https://github.com/DmytroKorniienko/FireLamp_JeeUI/tree/esp32) и ~~может немного отставать от мастера~~ пока работа приостановлена
