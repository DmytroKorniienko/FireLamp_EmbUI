...
В базовому варіанті використовується матриця розміром 16x16, 256 пікселів. Загалом проект гарантуе задовільну роботу матриць довільного розміру, з максимальною кількістю пікселів до 1024. 
Деякі відео, пов'язані з проектом, можна подивитися на [цьому]https://www.youtube.com/channel/UCMkXEbpQH7_xIXp_tFIdeag каналі YouTube.  
   ...
   - Також в наявності є багато композицій, які міняють поведінку, колір чи яскравість залежно від звуку. Є кілька спеціальних музичних ефектів типу "Осцилограф" та "Частотний аналізатор". 
   - Окрім цього, проект підтримує трансляції світлових композицій в протоколі E1.31 з програми Jinx! та websocket з модифікованої нами Soulmate IDE.
   - Проект можна оснастити DFPlayer для озвучування назв композицій, часу, будильника та просто програвання MP3 файлів відповідно композиції, або як звичайний  МП3 плеєр. 
   - В проекті є програвач анімаційних файлів нашого власного формата, з підтриммкою однобайтного кодування кольору RGB332 та двохбайтного - RGB565. Цей формат можна отримати за допомогою написаного нами інструменту з GIF-анімацій, попередньо обробивши GIF в онлайн редакторі. Інструмет, посилання на онлайн редактор, та якийсь набір GIF, чи готові файли, можна знайти за посиланням внизу цього тексту. Програвач можна найти внизу списка ефектів, за номером 251.
   - Проект також підтримує управління http-командами, та інтеграції на базі MQTT.
   - Підтримується спеціальний вихід на AUX-Реле, який управляється кліком в WebUI, або автоматично по часу виконання включення\виключення (спрощений аналог реле Sonoff).
   - Продвинута система автоматизацій в розділі, випливаючого зліва, меню WebUI "Події" - будильники, зміна ефектів, озвучення часу, зміна яскравості, стану AUX-Реле etc. 
   - Підтримка датчика темеператури DS18B20 та спецыального виходу управління вентилятором обдуву матриці. Цей вихід також може бути задіяним для інших дій по перевищенню температури (залежно від Вашої фантазії та потреб). Вивід температури в WebUI та на дисплей TM1637. (Оперативні зміни лімітів з WebUI поки не реалізовані, задаються в user_config.h до компіляції проекту)
